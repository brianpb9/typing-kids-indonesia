// Reskin QC screenshots — home / in-game easy / victory, desktop + mobile.
// Starts a static server on 4173, seeds localStorage like e2e/deep.spec.js.
...
async function shoot(browser, viewport, suffix) {
  ...
  await page.screenshot({ path: `test-results/reskin-home-${suffix}.png` });

  // In-game easy mode with OSK
  await page.locator('#start-btn').click();
  await page.waitForSelector('#game-screen:not(.hidden)', { timeout: 10_000 });
  await page.waitForTimeout(900);
  await page.screenshot({ path: `test-results/reskin-game-${suffix}.png` });
  ...
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.screenshot({ path: `test-results/reskin-victory-${suffix}.png` });
  await page.close();
}
...
try {
  ...
  await browser.close();
  console.log('screenshots saved to test-results/reskin-*.png');
} finally {
  server.kill();
}
